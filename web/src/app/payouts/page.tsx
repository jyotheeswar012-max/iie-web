...
const FEED = [
  ...
  { farmer:'Arun Patel',   district:'Amritsar',  amount:'₹44,600', status:'SUCCESS', trigger:'Drought',  time:'12:41' },
  { farmer:'Geeta Devi',   district:'Surat',     amount:'₹26,800', status:'SUCCESS', trigger:'Flood',    time:'12:30' },
]
...
export default function PayoutsPage() {
  ...
  const filtered = filter==='ALL' ? FEED : FEED.filter(f=>f.status===filter)
  const total = FEED.filter(f=>f.status==='SUCCESS').reduce((_s,_f)=>_s+parseInt(_f.amount.replace(/[₹,]/g,'')),0)
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <motion.div initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }}
        className="rounded-2xl p-6 mb-6" style={{ background:'linear-gradient(135deg,#0a0e27,#0d1b4b,#00695c)' }}>
        <div className="text-xs font-bold tracking-[3px] text-brand-teal uppercase mb-2">⚡ Auto-Payment Engine</div>
        <h1 className="text-3xl font-black gradient-text">Live Payout Dashboard</h1>
        <p className="text-white/60 text-sm mt-2">Real-time UPI/IMPS auto-credits to SBI savings accounts · Zero claims · Zero waiting</p>
      </motion.div>
      {/* KPIs */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ['₹',`₹${(total/100000).toFixed(1)}L`,'Today\'s Payouts','text-brand-teal'],
          ['✅',FEED.filter(f=>f.status==='SUCCESS').length,'Successful','text-brand-green'],
          ['⏳',FEED.filter(f=>f.status==='PENDING').length,'Pending','text-brand-orange'],
          ['⚡','47 min','Avg Settlement','text-brand-blue'],
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div className="bg-brand-card border border-brand-border rounded-2xl p-4">
          <h3 className="font-bold text-brand-text text-sm mb-3">📈 14-Day Payout Volume (₹ Cr)</h3>
        ...
        <div className="bg-brand-card border border-brand-border rounded-2xl p-4">
          <h3 className="font-bold text-brand-text text-sm mb-3">👨‍🌾 Farmers Paid Per Day</h3>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-brand-border">
          <h3 className="font-bold text-brand-text text-sm">⚡ Live Payout Feed</h3>
          <div className="flex gap-2">
            {['ALL','SUCCESS','PENDING'].map(f=>(
              <button key={f} onClick={()=>setFilter(f)}
                className={`text-[11px] font-bold px-3 py-1 rounded-lg transition ${
                  filter===f ? 'bg-brand-teal text-brand-navy' : 'text-brand-muted hover:bg-white/5'
                }`}>{f}</button>
            ))}
          </div>
        </div>
          ...
          {filtered.map((p,i) => (
            <motion.div key={i} initial={{ opacity:0,x:-10 }} animate={{ opacity:1,x:0 }}
              ...
              <div className="flex-1 min-w-0">
                <span className="font-bold text-brand-text text-sm">{p.farmer}</span>
                <span className="text-brand-muted text-xs ml-2">{p.district}</span>
              </div>
              <span className="text-xs text-brand-muted">{p.trigger}</span>
              <span className="font-black text-brand-green text-sm">{p.amount}</span>
              <span className="text-[11px] text-brand-muted font-mono">{p.time}</span>
            </motion.div>
          ))}
        </div>
      </div>
