    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 my-6">
      {METRICS.map((m, i) => (
        <motion.div key={i}
          ...
          className="glass p-4 text-center group hover:-translate-y-1 transition-transform cursor-default"
        >
          <div className="text-2xl mb-1">{m.icon}</div>
          <div className="text-2xl font-black" style={{ color: m.color }}>{m.value}</div>
          <div className="text-[10px] text-brand-muted uppercase tracking-widest mt-1">{m.label}</div>
          <div className="text-[10px] text-brand-green mt-1">▲ {m.delta}</div>
        </motion.div>
      ))}
    </div>
  )
