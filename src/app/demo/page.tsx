...
const SRC_COL: Record<OracleSource,string> = {
  live_today:'#4ade80', live_yesterday:'#fbbf24', cached_baseline:'#94a3b8', manual_override:'#a78bfa',
};
const SRC_LABEL: Record<OracleSource,string> = {
  live_today:'🛰️ live · today', live_yesterday:'🕐 live · yesterday', cached_baseline:'📦 baseline', manual_override:'✏️ manual',
};
    ...
    <Card>
      ...
      {train.confusion_matrix&&(
        <div style={{marginTop:10,display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7}} className="g4">
          {([
            ['TP',train.confusion_matrix.tp,'#4ade80'],
            ['FP',train.confusion_matrix.fp,'#fbbf24'],
            ['TN',train.confusion_matrix.tn,'#38bdf8'],
            ['FN',train.confusion_matrix.fn,'#f87171'],
...
function FSMPath({current,previous}:{current?:CState;previous?:CState}) {
  ...
  const stateIcon:Record<CState,string>={ACTIVE:'🟢',TRIGGERED:'⚡',FRAUD_REVIEW:'🕵️',EXECUTED:'✅',REJECTED:'❌'};
  return (
    <Card style={{background:'#0d1117',border:'1px solid #1e293b'}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}}>
        {states.map(s=>{
          ...
          const passed=previous===s
            ||(s==='ACTIVE'&&current&&current!=='ACTIVE')
            ||(s==='TRIGGERED'&&['EXECUTED','FRAUD_REVIEW','REJECTED'].includes(current||''));
...
function ClaimModal({exec,onClose,hindi}:{exec:ExecuteResult;onClose:()=>void;hindi:boolean}) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Payout confirmed"
      ...
      onClick={onClose}>
      <div style={{background:'#0d1117',border:'2px solid #166534',borderRadius:24,padding:'36px 32px',maxWidth:460,width:'100%',textAlign:'center',position:'relative',overflow:'hidden',boxShadow:'0 0 60px #4ade8044'}}
        ...
        <button onClick={onClose} style={{background:'linear-gradient(135deg,#065f46,#047857)',color:'#d1fae5',border:'none',borderRadius:10,padding:'11px 28px',fontSize:13,fontWeight:700,cursor:'pointer'}}>
...
export default function DemoPage() {
  ...
  const doExecute=async()=>{
    ...
    try{
      ...
      setExecute(d);setStep('audit');
      if((d.current_state??'EXECUTED')==='EXECUTED')setShowModal(true);
      ping();
    }catch(e:unknown){setError(e instanceof Error?e.message:'Error');}
    stopTimer();setLoading(false);
  };
    ...
    try{
      ...
      if(tdRaw){
        ...
        setTrain({
          ...
          metrics:{
            accuracy:gb.accuracy??0,precision:gb.precision??0,recall:gb.recall??0,
            f1:gb.f1_score??0,auc:gb.roc_auc??0,
            train_rows:ds.train_samples??0,val_rows:ds.test_samples??0,
  ...
  const inp=(extra?:React.CSSProperties):React.CSSProperties=>({
    width:'100%',border:'1px solid #1e293b',borderRadius:8,padding:'8px 11px',
    fontSize:13,background:'#030712',color:'#e2e8f0',transition:'border-color 0.2s',...extra,
  });
              ...
              <Card>
                ...
                <div style={{background:forceState==='FRAUD_REVIEW'?'#1c0a00':forceState==='REJECTED'?'#2d0a0a':'#052e16',border:`1px solid ${forceState==='FRAUD_REVIEW'?'#9a3412':forceState==='REJECTED'?'#7f1d1d':'#166634'}`,borderRadius:10,padding:'12px 14px'}}>
                  ...
                  <div style={{fontSize:12,color:'#94a3b8'}}>
                    {forceState==='FRAUD_REVIEW'?'Shows orange review path before final settlement.':forceState==='REJECTED'?'Shows failed transition and claim rejection path.':'Shows standard TRIGGERED → EXECUTED payout flow.'}
                  </div>
                </div>
              </Card>
            </div>
              ...
              <Card style={{marginBottom:12}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:18}} className="g4">
                  {Object.entries(verify.oracle_inputs).map(([key,v])=>{
                    ...
                    return (
                      <div key={key} style={{background:'#030712',border:`1px solid ${src==='live_today'?'#166534':'#1e293b'}`,borderRadius:8,padding:'9px 10px'}}>
                        ...
                        <div style={{display:'inline-block',background:`${srcCol}18`,color:srcCol,border:`1px solid ${srcCol}44`,borderRadius:4,padding:'1px 5px',fontSize:8,fontWeight:700,whiteSpace:'nowrap'}}>{srcLabel}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            {audit&&(
              <div style={{display:'flex',justifyContent:'flex-end'}}>
                <button onClick={doML} disabled={loading}
                  style={{background:'linear-gradient(135deg,#0c4a6e,#0369a1)',color:'#bae6fd',border:'none',borderRadius:10,padding:'11px 22px',fontSize:12,fontWeight:700