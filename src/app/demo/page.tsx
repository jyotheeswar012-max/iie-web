...
function AgentBar({ name, a, delay }: { name:string; a:Agent; delay:number }) {
  ...
  return (
    <div role="button" tabIndex={0} aria-expanded={open} onKeyDown={e=>e.key==='Enter'&&setOpen(o=>!o)} onClick={()=>setOpen(o=>!o)}
        ...
        {states.map(s=>{
          ...
          return (
            <div key={s} style={{ flex:'1 1 90px',minWidth:90,background:active?`${col}18`:'#030712',border:`${active?2:1}px solid ${active?col:passed?`${col}66`:'#1e293b'}`,borderRadius:10,padding:'10px 8px',textAlign:'center',boxShadow:active?`0 0 18px ${col}44`:undefined,animation:s==='FRAUD_REVIEW'&&active?'fraudPulse 1.2s ease-in-out infinite':undefined }}>
              <div style={{ fontSize:18 }}>{s==='ACTIVE'?'🟢':s==='TRIGGERED'?'⚡':s==='FRAUD_REVIEW'?'🕵️':s==='EXECUTED'?'✅':'❌'}</div>
        ...
        {step==='enroll'&&(
          <div className="fi">
            ...
            <div style={{ marginTop:14,display:'flex',justifyContent:'flex-end' }}>
              <button onClick={doEnroll} disabled={loading} style={{ background:loading?'#1e293b':'linear-gradient(135deg,#065f46,#047857)',color:loading?'#475569':'#d1fae5',border:'none',borderRadius:10,padding:'11px 26px',fontSize:13,fontWeight:700,display:'flex',alignItems:'center',gap:8,boxShadow:loading?'none':'0 4px 16px #065f4666' }}>{loading&&<Spin/>} {loading?(hindi?HI.loading:'Enrolling…'):(hindi?'🚀 पॉलिसी जारी करें':'🚀 Issue Policy & Deploy Contract')}</button>
            </div>
          </div>
        )}
