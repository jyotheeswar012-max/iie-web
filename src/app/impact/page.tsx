...
const COMPARE = [
  { metric:'Payout time',             pmfby:'47 days average',               iie:'2.8 seconds',                            delta:'99.993% faster',    source:'PMFBY Annual Report 2023-24, MoAFW',                                 color:C.green  },
  { metric:'Claim forms',             pmfby:'12 paper forms',                iie:'0 forms',                                delta:'100% eliminated',   source:'PMFBY State Implementation Guide 2023',                              color:C.teal   },
  { metric:'Fraud rate',              pmfby:'23% of claims',                 iie:'< 2% (AI quorum)',                       delta:'−91%',              source:'CAG Report on PMFBY, 2022–23',                                     color:C.orange },
  { metric:'Admin cost per policy',   pmfby:'₹4,800',                         iie:'₹38',                                   delta:'−99.2%',            source:'IRDAI Annual Report 2022–23, Table 4.3',                            color:C.amber  },
  { metric:'Farmer reach',            pmfby:'4.2 Cr enrolled',               iie:'14 Cr+ (SBI YONO base)',                 delta:'3.3× scale',        source:'PMFBY Dashboard, DAC&FW; SBI Annual Report 2023–24',               color:C.blue   },
  { metric:'Crop loss verification',  pmfby:'Manual field survey (30–45d)', iie:'Satellite + sensor quorum (1.2s)',       delta:'Fully automated',   source:'ISRO NRSC Remote Sensing Applications 2023',                        color:C.purple },
  { metric:'Audit trail',             pmfby:'Paper ledger, state-held',      iie:'Hyperledger Fabric, IRDAI-accessible',   delta:'Regulator-native',  source:'IRDAI (Digital Transformation) Regulation 2024',                    color:C.teal   },
  { metric:'KCC credit upsell',       pmfby:'Not possible',                  iie:'₹40K KCC top-up in same session',         delta:'SBI-exclusive moat',source:'SBI KCC Product Circular 2024; RBI Priority Sector Norms',          color:C.orange },
];
...
const METRICS = [
  ...
  { icon:'💰', value:'759 Cr',   label:'SBI Year 1 P&L value',      sub:'Float + KCC NIM + YONO + CAC savings — see SBI P&L →' },
...
function MetricCard({ m }: { m: typeof METRICS[0] }) {
  const isSBIPL = m.label === 'SBI Year 1 P&L value';
  return (
    <div style={{ borderRadius:18, border:`1px solid ${isSBIPL ? C.amber+'88' : C.border}`, background:isSBIPL ? `${C.amber}08` : C.panel, padding:'18px 20px', textAlign:'center' }}>
      <div style={{ fontSize:28, marginBottom:4 }}>{m.icon}</div>
      <div style={{ fontSize:30, fontWeight:900, color: isSBIPL ? C.amber : C.teal, lineHeight:1 }}>{m.value}</div>
      ...
      {isSBIPL && (
        <Link href="/sbi-pl" style={{ display:'inline-block', marginTop:8, fontSize:10, fontWeight:800, color:C.amber, textDecoration:'none', border:`1px solid ${C.amber}44`, borderRadius:8, padding:'3px 10px' }}>Full SBI P&L →</Link>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ background:C.bg, color:C.text, minHeight:'100vh', padding:'0 0 64px' }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 18px 0' }}>
        ...
        {/* SBI P&L callout banner */}
        <Link href="/sbi-pl" style={{ textDecoration:'none', display:'block', marginBottom:24 }}>
          <div style={{ borderRadius:18, padding:'18px 24px', background:'linear-gradient(135deg,rgba(227,179,65,0.08),rgba(100,255,218,0.06))', border:`1.5px solid ${C.amber}55`, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12, cursor:'pointer' }}>
            <div>
              <div style={{ fontSize:11, fontWeight:800, color:C.amber, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:4 }}>💰 New — SBI P&L Model</div>
              <div style={{ fontSize:17, fontWeight:900, color:C.text }}>What does SBI actually earn from IIE?</div>
              <div style={{ fontSize:12, color:C.sub, marginTop:4 }}>Premium float · KCC NIM · YONO lift · CAC savings · Commission · Retention LTV</div>
            </div>
            <div style={{ textAlign:'right' }}>
              <div style={{ fontSize:10, color:C.sub }}>Year 1 total value to SBI</div>
              <div style={{ fontSize:36, fontWeight:900, color:C.amber }}>₹759 Cr</div>
              <div style={{ fontSize:11, color:C.amber, fontWeight:700 }}>See full SBI P&L →</div>
            </div>
          </div>
        </Link>
        ...
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, padding:'22px 24px', marginBottom:28 }}>
          ...
          <div style={{ marginTop:16, padding:'12px 16px', borderRadius:12, background:'#0a1120', border:`1px solid ${C.border}` }}>
            <span style={{ fontSize:10, color:C.sub }}>Explainability method: SHAP values · Baseline: mean prediction across holdout set · Quorum trigger requires ≥75% weighted oracle agreement — no single data source can trigger payout.</span>
          </div>
        </div>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', justifyContent:'center' }}>
          {[
            ...
            { label:'💰 SBI P&L',    href:'/sbi-pl',  color:C.amber  },
