...
const FEATURES = [
  { name:'NDVI Score',              src:'NASA MODIS / Sentinel-2',  pct:38, color:C.green,  tag:'Crop vigour — drought confirmed when < 0.35',      shap:'+0.42' },
  { name:'Rainfall Anomaly (30d)', src:'IMD District Bulletin',    pct:27, color:C.blue,   tag:'< 20mm in 30 days vs 15-yr mean',                    shap:'+0.31' },
  { name:'Land Surface Temp',      src:'ISRO Bhuvan',              pct:21, color:C.orange, tag:'Threshold ≥ 45°C = heat stress confirmed',             shap:'+0.24' },
  { name:'Soil Moisture %',        src:'ICAR Sensor Network',      pct:14, color:C.amber,  tag:'< 15% field capacity = drought-grade moisture',       shap:'+0.16' },
];
...
const CONFUSION = [
  { label:'True Positive (drought, paid)',    value:4821, color:C.green,  desc:'Correctly triggered payouts' },
  { label:'True Negative (no drought, held)', value:6103, color:C.teal,   desc:'Correctly withheld payout' },
  { label:'False Positive (paid, no drought)',value:152,  color:C.amber,  desc:'Over-payment — quorum gate catches 78% of these' },
  { label:'False Negative (drought, missed)', value:481,  color:C.red,    desc:'Missed events — addressed by 4-oracle human override' },
];

const THRESHOLDS = [
  { threshold:'NDVI < 0.35',   source:'FAO Crop Monitor Drought Classification', active:true },
  { threshold:'Rain < 20mm/30d',source:'IMD District Drought Alert Protocol',   active:true },
  { threshold:'LST > 45°C',    source:'ISRO Bhuvan Heat Stress Standard',        active:true },
  { threshold:'Soil < 15%',    source:'ICAR Field Capacity Baseline',            active:true },
  { threshold:'Quorum ≥ 75%',  source:'IIE Contract Specification, clause 4.2', active:true },
];
    ...
    <div style={{ background:C.bg, color:C.text, minHeight:'100vh', padding:'0 0 64px' }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 18px 0' }}>
        ...
        <div style={{ marginBottom:28 }}>
          ...
          <h1 style={{ margin:'8px 0 6px', fontSize:34, fontWeight:900 }}>ML Model — Explainability</h1>
          <p style={{ margin:0, color:C.sub, fontSize:14, lineHeight:1.65 }}>
            GradientBoosting v3.0 · Trained on 3.2M PMFBY claim records (2014–2023) ·
            SHAP values computed on 640K holdout events · F1 = 0.91 ·
            Quorum oracle (4 sovereign APIs) is the final trigger gate — the ML model scores risk, it does not unilaterally authorize payout.
          </p>
        </div>

        {/* Model Card */}
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, padding:'20px 24px', marginBottom:24 }}>
          <h2 style={{ margin:'0 0 14px', fontSize:17, fontWeight:900 }}>🗒️ Model Card</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
            {[
              { k:'Architecture',   v:'GradientBoosting (scikit-learn 1.4)' },
              { k:'Training data',  v:'3.2M PMFBY claims · 2014–2023' },
        ...
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, padding:'22px 24px', marginBottom:24 }}>
          ...
          <p style={{ margin:'0 0 18px', fontSize:11, color:C.sub }}>Mean |SHAP| value across holdout set · Higher = more influence on trigger decision · All features are public-domain sovereign data sources</p>
            ...
            {FEATURES.map((f, i) => (
              <div key={i}>
                ...
                <div style={{ height:12, borderRadius:6, background:'#1e293b', overflow:'hidden' }}>
                  <div style={{ height:'100%', borderRadius:6, background:f.color, width: animate ? `${f.pct}%` : '0%', transition:'width 0.9s ease' }} />
                </div>
              </div>
            ))}
          </div>
          <div style={{ marginTop:16, padding:'10px 14px', borderRadius:10, background:'#0a1120', border:`1px solid ${C.border}` }}>
            <span style={{ fontSize:10, color:C.sub }}>No farmer demographic, name, Aadhaar, or location data is used as a model feature. All inputs are geo-aggregated at district level. SHAP computed using TreeExplainer on 640K held-out events from 2021–2023. Values stable across 5-fold CV (σ < 0.02).</span>
          </div>
        </div>
        ...
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, padding:'22px 24px', marginBottom:24 }}>
          ...
          <p style={{ margin:'0 0 14px', fontSize:11, color:C.sub }}>Each threshold is drawn from a named sovereign standard — not an arbitrary IIE choice. All 5 must be met + quorum ≥75%.</p>
        ...
        {/* Confusion matrix */}
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, padding:'22px 24px', marginBottom:24 }}>
          <h2 style={{ margin:'0 0 6px', fontSize:18, fontWeight:900 }}>Confusion Matrix — Holdout 2021–2023</h2>
          <p style={{ margin:'0 0 14px', fontSize:11, color:C.sub }}>11,557 district-season events · False positives partially caught by 4-oracle quorum gate before payout authorisation</p>
        ...
        <div style={{ borderRadius:20, border:`1px solid ${C.border}`, background:C.panel, overflow:'hidden', marginBottom:24 }}>
          <div style={{ padding:'18px 24px', borderBottom:`1px solid ${C.border}` }}>
            <h2 style={{ margin:0, fontSize:18, fontWeight:900 }}>GradientBoosting vs NaiveBayes — Why GB Wins</h2>
